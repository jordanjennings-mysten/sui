...
mod checked {
            ...
            if !self.protocol_config.normalize_ptb_arguments() {
                ...
                for (arg_idx, arg) in args.enumerate() {
                    self.splat_arg(&mut res, arg)
                        .map_err(|e| Mode::Error::from(e.into_execution_error(start_idx + arg_idx)))?;
                }
                ...
                for (i, command_result) in results.iter().enumerate() {
                    for (j, result_value) in command_result.iter().enumerate() {
                        ...
                        match value {
                            None => (),
                            Some(Value::Object(_)) => {
                                return Err(Mode::Error::from_kind(ExecutionErrorKind::UnusedValueWithoutDrop {
                                    result_idx: i as u16,
    ...
    impl<'vm, 'state, 'a, Mode: ExecutionMode> TypeTagResolver for ExecutionContext<'vm, 'state, 'a, Mode> {
    ...
    fn load_object<Mode: ExecutionMode>(
        ...
        let obj_value = value_from_object::<Mode>(protocol_config, vm, linkage_view, new_packages, obj)?;
        let contained_uids = {
    ...
    fn convert_type_argument_error<S: MoveResolver<Err = SuiError>, Mode: ExecutionMode>(
        ...
        match error.major_status() {
            ...
            StatusCode::TYPE_RESOLUTION_FAILURE => Mode::Error::from_kind(ExecutionErrorKind::TypeArgumentError {
                argument_idx: idx as TypeParameterIndex,
                kind: TypeArgumentError::TypeNotFound,
            }),
            StatusCode::CONSTRAINT_NOT_SATISFIED => Mode::Error::from_kind(ExecutionErrorKind::TypeArgumentError {
                argument_idx: idx as TypeParameterIndex,
                kind: TypeArgumentError::ConstraintNotSatisfied,
            }),
            _ => convert_vm_error::<_, Mode>(error, vm, state_view, resolve_abort_location_to_package_id),
        }
    }
}
